<template>
  <div class="flex items-center space-x-1">
    <button
      ...
      class="text-lg transition-colors duration-150 focus:outline-none"
      :class="{
    ...
    <span v-if="showText" class="ml-2 text-sm text-gray-600">
      {{ currentRating ? `${currentRating}/5` : 'No rating' }}
    </span>
  </div>
</template>
...
const handleRatingClick = (star) => {
  ...
  const newRating = currentRating.value === star ? null : star
